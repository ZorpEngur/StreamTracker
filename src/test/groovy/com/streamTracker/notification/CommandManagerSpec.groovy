...
class CommandManagerSpec extends Specification {
    ...
    void "parser register command"() {
        ...
        then:
        1 * userService.saveUser(DatabaseUserModel.builder()
                .twitchId(twitchId)
    ...
    void "successfully parser set command"() {
        given:
        UserService userService = Mock() {
            getUser("First User") >> Mock(DatabaseUserModel) {
                getId() >> 1
                getName() >> "First User"
            }
            getTwitchUser(1) >> Mock(DatabaseUserModel) {
                getId() >> 2
                getName() >> "name"
            }
        }
    ...
    void "generated tests"() {
        ...
        UserService userService = Mock() {
            ...
            getUser("name") >> Mock(DatabaseUserModel) {
                getId() >> 2
            }
        }
        TwitchBotService twitchBotService = Mock()
