...
public class CommandManager {
    ...
    private String register(@NonNull Iterator<String> parameters, @Nullable Long twitchId, @NonNull String name) throws CommandException {
        DatabaseUserModel.DatabaseUserModelBuilder builder = DatabaseUserModel.builder()
                .name(name)
                .twitchId(twitchId);
        ...
        while (parameters.hasNext()) {
            ...
            switch (Parameters.fromValue(param[0].toLowerCase())) {
                case DISCORD:
                    if (param.length != 2 || !param[1].matches("^\\d+$")) throw new CommandException("Invalid discord ID.");
        ...
        this.userService.saveUser(builder.build());
        return "User registered. " + builder.build().getName();
    }
    ...
    private String set(@NonNull Iterator<String> parameters, @Nullable Long twitchId) throws CommandException {
        ...
        if (twitchId != null) {
            DatabaseUserModel user = this.userService.getTwitchUser(twitchId);
        ...
        while (parameters.hasNext()) {
            ...
            switch (Parameters.fromValue(param[0].toLowerCase())) {
                case USER:
                    if (param.length == 2) {
                        DatabaseUserModel user = this.userService.getUser(param[1]);
                        if (user == null || user.getId() == null) throw new CommandException("User not found.");
                        builder.userId(user.getId());
                        userName = user.getName();
                    }
                    break;
                ...
                case PLATFORM:
                    if (param.length == 2) {
                        ...
                        if (platform == null) throw new CommandException("Invalid notification platform.");
    ...
    private String help(@NonNull Iterator<String> parameters) throws CommandException {
        ...
        if (all || command.equals("register")) {
            answer.append("register: creates new user for this bot. Parameters:\n");
            answer.append(Parameters.DISCORD);
            answer.append(Parameters.NAME);
            answer.append(Parameters.NOTME);
        }
        if (all || command.equals("set")) {
            answer.append("set: adds new notification event for registered user. Parameters:\n");
